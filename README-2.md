...
- **Jump straight to the email.** Each card has **Open in Gmail ↗** (opens the most recent
  message for that application), and every row under **History** is itself a link to that
  thread in Gmail. Links open in a new tab and resolve to the account you synced with, so
  they work even if you're signed into several Gmail accounts at once.
